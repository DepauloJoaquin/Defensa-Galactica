class EstadoAlerta extends Estado{
  update(dt){
    ...
    if (p.moverHaciaJugador) p.moverHaciaJugador(dt, p.velRapida);
    if (p?.cambiarAnimacion) p.cambiarAnimacion(p._animKey?.('correr') || 'correrAbajo');
  ...
  return;
    ...
    if (d2 <= p.rangoAtaqueJugadorCuad) this.fsm.setear('AGGRESSIVE_PLAYER');
    else if (d2 > p.rangoAlertaCuad * 1.4) this.fsm.setear('INTRIGADO');
